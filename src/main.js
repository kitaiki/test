...
document.getElementById('btn-multi').addEventListener('click', () => {
  ...
  createAddPolyline(coords, 50, 'both'); // 50m 거리로 평행선 생성
  
  alert(`
    베어링(방위각): ${bearing.toFixed(2)}도
    거리: ${distance.toFixed(2)}km
    
    폴리곤이 50m 양쪽에 생성되었습니다.
...
function createAddPolyline(coords, distance, type) {
  ...
  // 각 점에서 정확한 수직 거리를 계산하여 평행선 생성
  const leftCoords = [];
  const rightCoords = [];
  
  for (let i = 0; i < wgs84Coords.length; i++) {
    const currentPoint = wgs84Coords[i];
    let perpendicularBearing;
    
    if (i === 0) {
      // 첫 번째 점: 다음 점으로의 방향에 수직
      const bearing = turf.bearing(currentPoint, wgs84Coords[i + 1]);
      perpendicularBearing = bearing;
    } else if (i === wgs84Coords.length - 1) {
      // 마지막 점: 이전 점으로부터의 방향에 수직
      const bearing = turf.bearing(wgs84Coords[i - 1], currentPoint);
      perpendicularBearing = bearing;
    } else {
      // 중간 점: 이전 점과 다음 점을 고려한 각 이등분선에 수직
      const prevPoint = wgs84Coords[i - 1];
      const nextPoint = wgs84Coords[i + 1];
